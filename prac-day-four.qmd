...
avg_arrival <- flights |>   
  ...
  filter(paste(origin, dest) %in% paste(mult_flights$origin, mult_flights$dest)) |>  # Filter for identified routes  
  group_by(origin, dest, carrier) |>   # Group by origin, destination, and carrier  
  summarise(mean_arr_delay = mean(arr_delay, na.rm = TRUE))  # Calculate mean arrival 

avg_arrival

#%in% operator checks if elements on the left side are present in the vector on the right.
...
best_worst <- avg_arrival |>   
  ...
  summarise(best_airline = carrier[which.min(mean_arr_delay)],  
            ...
            difference = max(mean_arr_delay) - min(mean_arr_delay), .group = 'drop') 
